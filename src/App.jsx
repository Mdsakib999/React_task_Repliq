    ...
    <div className="flex ml-2 ">
      ...
      {selectedProducts.map((product) => (
          <div key={product.id} className=" flex items-center">
            ...
            <SlNote className="text-3xl "></SlNote>
            <div className="flex items-center border-r-2 border-l-2 border-b-2 ml-3 px-2 py-2 text-gray-500 text-xl">
            <p className="mr-56">{product.name}</p>
            <p className="mr-10">${product.price}</p>
            <FaMinusCircle onClick={() => handleRemoveQuantity(product.id)} className="mr-8 cursor-pointer" />
            {/* product quantity */}
            <p className="mr-2">{product.quantity}</p>
            <IoIosAddCircle onClick={() => handleAddQuantity(product.id)} className="mr-8 ml-7 text-2xl cursor-pointer" />
            <p className="">${(product.quantity * product.price).toFixed(2)}</p>
            </div>
            <MdOutlineDeleteForever className="text-red-600 cursor-pointer ml-6 text-3xl" onClick={() => handleRemoveProduct(product.id)} />
          </div>
        ))}
      ...
      <div className="flex">
        ...
        <div className="w-[45%] mt-5 ">
          ...
          <div className="flex justify-between my-2">
            <p>Sub Total</p>
            <p className="font-bold">${(subTotal +181).toFixed(2)}</p>
          </div>
          <hr />
          <div className="flex justify-between my-2">
            <p>TAX</p>
      ...
      {/* total money */}
      <div className=" flex items-center justify-between px-5 py-3 mt-4 rounded-md bg-indigo-50 text-indigo-500">
        <p>Products Count ( {selectedProducts.length} )</p>
        <p className="text-xl font-semibold">Total</p>
        <p className="text-xl font-semibold">${(subTotal +206).toFixed(2)}</p>
      </div>
    ...
    {/* last button */}
      ...
      <div className="w-[50%] bg-gray-100  border-l-2 ">
        ...
        {products.map((product) => (
            <div key={product.id} className="border-2 border-gray-300" onClick={() => addToSelectedProducts(product)}>
              <div className="">
